 ...
 */
var Events_API_Keyword = {
	API_URI_GET : "https://www.satakuntaevents.fi/api/v2/keyword/?",
	PARAMETER_PAGE : "page",
	 ...
	 */
	createKeywordURI : function(baseUri, page, pageSize, sort, text){
		var uri = baseUri;
		var notFirst = false;

		if(page){
			uri += Events_API_Keyword.PARAMETER_PAGE+"="+page;
			notFirst = true;
		}
		...
		if(pageSize){
			...
			uri += Events_API_Keyword.PARAMETER_PAGE_SIZE+"="+pageSize;
			notFirst = true;
		}
		...
		if(sort){
			...
			uri += Events_API_Keyword.PARAMETER_SORT+"="+encodeURIComponent(sort);
			notFirst = true;
		}
		...
		if(text){
			...
			uri += Events_API_Keyword.PARAMETER_TEXT+"="+encodeURIComponent(text);
		}
	...
	get : function(callback, page, pageSize, sort, text){
		jQuery.ajax({
			url : Events_API_Keyword.createKeywordURI(Events_API_Keyword.API_URI_GET, page, pageSize, sort, text),
